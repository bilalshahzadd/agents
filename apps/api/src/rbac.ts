...
export function requireRole(min:'analyst'|'editor'|'approver'|'admin'|'owner'){ return async(req:FastifyRequest,reply:FastifyReply)=>{ await authenticate(req,reply); if(reply.sent) return; if((rank[req.actor!.role]??0)<(rank[min]??0)) return reply.code(403).send({error:'forbidden',message:`${min} role required`}); }; }
